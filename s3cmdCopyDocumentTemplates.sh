...
s3cmd -r -c "$SOURCE_CONFIG" get s3://"${BUCKET_PREFIX}/document_files" "${TMP_DIR}/documentfiles"
if [ $? -ne 0 ]; then
...
s3cmd -r -c "$DEST_CONFIG" put "${TMP_DIR}/document_files" s3://"${BUCKET_PREFIX}/document_files/"
if [ $? -ne 0 ]; then
